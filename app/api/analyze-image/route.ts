...
export async function POST(request) {
  try {
    ...
    const prompt = `
      ...
      Example JSON format:
      [
        { "id": 1, "description": "Wash your hands", "audioUrl": "/audio/wash-hands.mp3" },
        { "id": 2, "description": "Pick up the spoon", "audioUrl": "/audio/pick-spoon.mp3" }
      ]
    ...
    const analysis = response.text().match(/\[.*\]/s); // Extract JSON array from response
